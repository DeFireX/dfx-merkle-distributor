...
const TOTAL_DFX_DISTRIBUTION = 2000; // 2000 DFX for BSC
const MIN_BALANCE = 5;
const MAX_BLOCK = 5611923; // Mar-12-2021 09:15:41 AM +UTC
...
(async () => {
    ...
    if (!IS_GENERATE_TREE_ONLY) {
        ...
        fs.writeFileSync('./bscDfxDistributionShare.tmp', JSON.stringify({
        ...
        const rows = fs.readFileSync('./bscDfxDistributionFinal.tmp', 'utf8').split('\r\n').filter(x => x).map(x => x.trim());
        for(let row of rows) {
